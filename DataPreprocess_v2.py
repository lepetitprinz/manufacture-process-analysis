...
class DataPrepVerTwo(object):
    ...
    RN_WORK_REV = {'MOLD_NO': 'mold_id', 'WORK_CNT': 'mnt', 'PROC_CT': 'proc_nm', 'PROC_CT_1': 'proc_nm_rev',
               'WORK_CT': 'work_info', 'WORK_CT_1': 'work_info_rev', 'CAND_PROC_LC_CDM': 'proc_kind_cand',
               'CAND_PROC_MC_CDM': 'proc_kind_dtl_cand', 'WORKER_NM': 'worker_nm', 'WORK_DAY': 'work_day',
               'WORK_TIME': 'work_time', 'PROC_LC_CDM': 'proc_kind', 'PROC_MC_CDM': 'proc_kind_dtl'}
    ...
    def run(self):
        work = pd.read_csv(os.path.join(self.path_data, 'work_rev.csv'), delimiter='\t')
        work_time = pd.read_csv(os.path.join(self.path_data, 'work_time.csv'), delimiter='\t')

        # Rename columns
        work = work.rename(columns=self.__class__.RN_WORK_REV)
        ...
        # Save dataset
        merged_worker.to_csv(os.path.join(self.path_save, 'work_by_worker.csv'), index=False, sep='\t',
                             encoding='utf-8')

        merged_mold = merged_worker.drop(columns=['work_day', 'work_time_start', 'work_time_end'])
        merged_mold = merged_mold[['mold_id', 'make_time_end_bf', 'make_time_end', 'proc_nm', 'proc_nm_rev', 'mnt',
                                   'work_info', 'work_info_rev', 'proc_kind', 'proc_kind_dtl',
                                   'proc_kind_cand', 'proc_kind_dtl_cand']]
        merged_mold = merged_mold.sort_values(by=['mold_id', 'make_time_end_bf', 'make_time_end'])

        merged_mold.to_csv(os.path.join(self.path_save, 'work_by_mold.csv'), index=False, sep='\t',
                           encoding='utf-8')
...
def main():
    data_prep_ver_two = DataPrepVerTwo()
    data_prep_ver_two.run()
